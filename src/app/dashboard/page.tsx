...
export default function DashboardPage() {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/ai-insights', {
        ...
        body: JSON.stringify({ energy: 78, sleep: 82, stress: 45, immunity: 85 }),
      });
      if (res.ok) {
  ...
  const maxBar = Math.max(...weeklyData.map((d) => Math.max(d.energy, d.sleep)));
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 px-6 lg:px-8 py-10 md:py-14">
        ...
        <div className="relative mx-auto max-w-7xl flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">yihui 的健康档案</h1>
