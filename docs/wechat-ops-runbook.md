...
`wechat:check` is a local advisory inventory. `wechat:check:draft` is the hard
gate for draft upload and requires `md2wechat`, Official Account credentials,
and a cover. `wechat:check:production` is the PDD-guided Mini Program gate and
does not require WeChat Pay. `wechat:check:pay` is reserved for a future payment
launch.
...
The first Mini Program release uses PDD guided redirect only. PDD guided
redirect is controlled by:
...
When `PDD_SHORT_LINK_MODE=mini_program`, production readiness requires both PDD Mini Program values. If the values are missing or WeChat review blocks direct launch, use `web_bridge` or `copy_link` as fallback. Mini Program product APIs must not expose raw `pddUrl`.
