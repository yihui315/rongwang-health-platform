...
test("WeChat operations runbook documents installed skills, PDD fallback, and auto-publish guardrails", () => {
  ...
  const serviceRoadmap = fs.readFileSync("docs/user-auth-knowledge-service-roadmap.md", "utf8");
  ...
  assert.match(runbook, /website mall/);
  assert.match(runbook, /\/checkout/);
  assert.match(miniProgramPlan, /PDD_MINIPROGRAM_APPID/);
  assert.match(miniProgramPlan, /pages\/mall-bridge\/index/);
  assert.match(miniProgramPlan, /\/products\/\[slug\]/);
  assert.match(miniProgramPlan, /navigateToMiniProgram/);
  assert.match(serviceRoadmap, /auth-wechat-miniprogram/);
  assert.match(serviceRoadmap, /vercel:auth/);
  assert.match(serviceRoadmap, /AssessmentReport/);
  assert.match(serviceRoadmap, /HealthKnowledgeEntry/);
